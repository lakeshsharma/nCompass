...
CPollerFailureCodeMessage::CPollerFailureCodeMessage(CMeters *meters,
		byte transCode,
		DWORD uniqueId,
		const byte * cardID,
		DWORD machineNumber,
		__int64 failureCode)
{
	...
	m_bufferSize = SIZE_OF_MESSAGE_HEADER + 
		...
		sizeof(byte) + sizeof(byte) + sizeof(__int64);  //failure code

	m_lpBuffer = new byte[m_bufferSize];
	m_bufferIsSet = false;
}

CPollerFailureCodeMessage::~CPollerFailureCodeMessage(void)
{
}

void CPollerFailureCodeMessage::SetStatus(DWORD status)
{
	m_status = status;
}

DWORD CPollerFailureCodeMessage::GetStatus()
{
	return m_status;
}

byte * CPollerFailureCodeMessage::GetReadWriteBuffer(DWORD &bufferSize)
{
	bufferSize = m_bufferSize;
	return m_lpBuffer;
}

void CPollerFailureCodeMessage::SetReadWriteBuffer()
{
	m_bufferIsSet = true;
	...
	// Failure Code
	transDataType = DATA_TYPE_FAILURE_CODE;
	...
	memcpy_s(m_lpBuffer + offset, m_bufferSize - offset, &m_failureCode, sizeof(m_failureCode));
	offset += sizeof(m_failureCode);
