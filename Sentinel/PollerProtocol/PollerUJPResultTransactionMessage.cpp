...
CPollerUJPResultTransactionMessage::CPollerUJPResultTransactionMessage(CMeters *meters,
	byte transType,
	DWORD uniqueID,
	const byte * cardID,
	DWORD jackpotID,
	...
	__int64 amount)
{
	...
	m_bufferSize = SIZE_OF_MESSAGE_HEADER +
		...
		sizeof(byte) + sizeof(byte) + sizeof(m_jackpotID) + //JackpotID
		sizeof(byte) + sizeof(byte) + sizeof(m_jackpotStatus) +  //Jackpot status
...
CPollerUJPResultTransactionMessage::~CPollerUJPResultTransactionMessage()
{
}

void CPollerUJPResultTransactionMessage::SetStatus(DWORD status)
{
	m_status = status;
}

DWORD CPollerUJPResultTransactionMessage::GetStatus()
{
	return m_status;
}

byte * CPollerUJPResultTransactionMessage::GetReadWriteBuffer(DWORD &bufferSize)
{
	bufferSize = m_bufferSize;
	return m_lpBuffer;
}

void CPollerUJPResultTransactionMessage::SetReadWriteBuffer()
{
	m_bBufferIsSet = true;
	...
	byte dataSize = sizeof(m_jackpotID);
	memcpy_s(m_lpBuffer + offset, m_bufferSize - offset, &dataSize, sizeof(dataSize));
	offset += sizeof(dataSize);

	memcpy_s(m_lpBuffer + offset, m_bufferSize - offset, &m_jackpotID, sizeof(m_jackpotID));
	offset += dataSize;

	// Jackpot Status
	transDataType = DATA_TYPE_UNATTENDED_JACKPOT_STATUS;
	memcpy_s(m_lpBuffer + offset, m_bufferSize - offset, &transDataType, sizeof(transDataType));
	offset += sizeof(transDataType);

	dataSize = sizeof(m_jackpotStatus);
	memcpy_s(m_lpBuffer + offset, m_bufferSize - offset, &dataSize, sizeof(dataSize));
	offset += sizeof(dataSize);

	memcpy_s(m_lpBuffer + offset, m_bufferSize - offset, &m_jackpotStatus, sizeof(m_jackpotStatus));
	offset += dataSize;
