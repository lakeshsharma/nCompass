...
const int MAX_WRITE_LEN = 80; /* maximum bytes to write */
const int MAX_READ_LEN = 80;  /* maximum bytes to read */
...
int open_port(char *port) {
	...
	if (fd == -1) {
		...
		snprintf(logmsg, 255, "Unable to communicate to device, is it plugged in, is the port correct, is it a RadBeacon v2.2+ and does user have serial port permissions?");
...
int send_cmd_with_delay(char *portname, uint8_t *write_bytes, int writelen, uint8_t *read_bytes, int delay)
{
	...
	else
	{
		...
		snprintf(errmsg, 255, "File Descriptor is unset");
		rblog(RBLOG_ERROR, errmsg);
		exit_code = -6;
	}
