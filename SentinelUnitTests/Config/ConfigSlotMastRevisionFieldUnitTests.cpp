...
{
	class ConfigSlotMastRevisionField : public ::testing::Test
	{
	public:
	};
		...
		///     CConfigSlotMastRevisionField::CConfigSlotMastRevisionField().
		/// </summary>
		TEST(ConfigSlotMastRevisionField, CConfigSlotMastRevisionFieldTest)
		{
			// Setup.
			const CConfigSlotMastRevisionField configField;
			const FieldTypes typeExpected = SLOTMAST_REVISION;
		...
		/// <summary>
		/// Test the implementation of:
		///     CConfigSlotMastRevisionField::SetSlotMastRevision()
		///     CConfigSlotMastRevisionField::GetSlotMastRevision()
		/// </summary>
		TEST(ConfigSlotMastRevisionField, SetSlotMastRevisionTest)
		{
			typedef struct
			{
				// Inputs.
				...
				// Expected results.
				const DWORD m_slotMastRevisionExpected;
			} TestDataType;
			...
			{
				// Setup.
				CConfigSlotMastRevisionField varConfigField;

				// Perform operation(s) to be tested.
				varConfigField.SetSlotMastRevision(TEST_DATA[testIndex].m_buffer, TEST_DATA[testIndex].m_length);
				const CConfigSlotMastRevisionField &configField(varConfigField);

				// Test.
				const WORD slotMastRevisionResult = configField.GetSlotMastRevision();

				{
					// Report #1.
					bool areEqual = slotMastRevisionResult == TEST_DATA[testIndex].m_slotMastRevisionExpected;
					string failedMsg;
					failedMsg = FormatString(_T("FAILED slotMastRevisionResult=%u expected=%u testIndex=%d in %s"),
						(unsigned)slotMastRevisionResult,
						(unsigned)TEST_DATA[testIndex].m_slotMastRevisionExpected,
						(int)testIndex,
						static_cast<LPCTSTR>(__TFUNCTION__));
					ASSERT_TRUE(areEqual) << failedMsg;
				}
