						...
						stage(systemTarget + ' Coverity') {
							warnError(systemTarget + ' coverity failed') {
								sh '''#!/usr/bin/env bash
									...
									export NCOMPASS_COVERITY_STREAM="${JOB_NAME// /-}" # "nCompass-Dev-2.8.SQ20", etc

									export NCOMPASS_COVERITY_USER="$NCOMPASS_COVERITY_USER@ARISTOCRAT-INC.COM" # "SAUS-JenkinsSVN@ARISTOCRAT-INC.COM" is the real Coverity username

									bash build-with-docker.sh coverity 2>&1 | tee -a build/sentinel-build.log
									bash build-with-docker.sh cov-commit-defects
								'''
							}
						}
					}
